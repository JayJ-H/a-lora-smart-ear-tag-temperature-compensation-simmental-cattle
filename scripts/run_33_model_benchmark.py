...
DEFAULT_INPUT = SHARED_ROOT / "outputs" / "analysis" / "benchmark33" / "benchmark_input.csv"
DEFAULT_OUTPUT_DIR = SHARED_ROOT / "outputs" / "analysis" / "benchmark33"
PROPOSED_MODEL_NAME = "TH-SHRC"
...
# Filled from the frozen 520-row input in deterministic lexical order.
...
def load_data(path: Path) -> pd.DataFrame:
    ...
    if len(df) != 520 or df["CowKey"].nunique() != 30:
        raise ValueError("Expected 520 rows and 30 anonymous CowKey values.")
...
def summary_row(
    ...
    return {
        ...
        "FoldMode": "within_cattle_random5_seed_partition",
    ...
    return pd.DataFrame(
        {
            ...
            "ModelFamily": "three-branch stacked compensation",
            "BenchmarkGroup": "proposed_model_reference",
            "FeatureSet": "fold_aligned_th_shrc_prediction",
...
def write_markdown(
    ...
    rows = [
        "# 33-model benchmark",
        "",
        f"Models: {conventional_count + evolution_count + 1}; rows per model: {row_count}; elapsed: {elapsed_seconds:.1f} s.",
        "",
...
def verify_benchmark_outputs(
    ...
    summary_models = set(summary_df["Model"].astype(str))
    prediction_models = set(prediction_df["Model"].astype(str))
    inventory_models = set(inventory_df["Model"].astype(str))
    model_sets_match = summary_models == prediction_models == inventory_models

    per_model = prediction_df.groupby("Model").agg(
        Rows=("RowID", "size"),
        UniqueRowIDs=("RowID", "nunique"),
        FiniteActual=("Actual", lambda values: bool(np.isfinite(values.to_numpy(float)).all())),
        FinitePredicted=("Predicted", lambda values: bool(np.isfinite(values.to_numpy(float)).all())),
    )
    incomplete_models = per_model.loc[
        (per_model["Rows"] != expected_rows)
        | (per_model["UniqueRowIDs"] != expected_rows)
        | ~per_model["FiniteActual"]
        | ~per_model["FinitePredicted"]
    ].index.astype(str).tolist()

    proposed = prediction_df.loc[prediction_df["Model"] == PROPOSED_MODEL_NAME].sort_values("RowID")
    source = source_df.sort_values("RowID")
    proposed_row_ids_match = proposed["RowID"].astype(int).tolist() == source["RowID"].astype(int).tolist()
    proposed_prediction_max_abs_difference = (
        float(np.max(np.abs(proposed["Predicted"].to_numpy(float) - source["Predicted"].to_numpy(float))))
        if proposed_row_ids_match and len(proposed) == len(source)
        else float("inf")
    )
    status = "PASS" if (
        actual_count == expected_model_count
        and model_sets_match
        and not failed_models
        and not incomplete_models
        and proposed_row_ids_match
        and proposed_prediction_max_abs_difference <= 1e-8
    ) else "FAIL"
    return {
        "status": status,
        "validation_design": "fold-seed-aligned same-cow random five-fold OOF",
        "rows_per_model": expected_rows,
        "expected_model_count": expected_model_count,
        "retrained_model_count": actual_count,
        "model_sets_match": model_sets_match,
        "failed_models": failed_models,
        "incomplete_models": incomplete_models,
        "proposed_row_ids_match": proposed_row_ids_match,
        "proposed_prediction_max_abs_difference_C": proposed_prediction_max_abs_difference,
        "per_model_completeness": per_model.reset_index().to_dict(orient="records"),
    }
...
def runtime_environment(seed: int) -> dict[str, object]:
    ...
    return {
        ...
        "packages": packages,
        "random_seed": seed,
        "outer_validation": "fold-seed-aligned same-cow random five-fold OOF",
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    ref_summary = summary_row(
        PROPOSED_MODEL_NAME,
        "three-branch stacked compensation",
        "proposed_model_reference",
        "fold_aligned_th_shrc_prediction",
        reference,
        0.0,
        "Saved TH-SHRC OOF predictions aligned to this benchmark fold seed.",
    )
    ...
    inventory.append(
        {
            ...
            "FeatureSet": "fold_aligned_th_shrc_prediction",
            "RunStatus": "done",
            "Notes": "Reference model from fold-aligned single-seed TH-SHRC OOF predictions.",
        }
    )

    conventional_specs = conventional_model_specs()
    if len(conventional_specs) != 26:
        raise RuntimeError(
            f"The frozen benchmark requires 26 conventional models, got {len(conventional_specs)}"
        )
    ...
    if not args.skip_evolution and len(evolution_specs) != 6:
        raise RuntimeError(
            f"The frozen benchmark requires 6 evolutionary models, got {len(evolution_specs)}"
        )
    ...
    class_metrics_path = tables_dir / "15_model_metrics_by_data_class.csv"
    ...
    classified = prediction_df.merge(
        df[["RowID", "DataClass"]], on="RowID", validate="many_to_one"
    )
    class_rows = []
    for (model, model_family, benchmark_group, data_class), group in classified.groupby(
        ["Model", "ModelFamily", "BenchmarkGroup", "DataClass"], sort=True
    ):
        class_rows.append(
            {
                "Model": model,
    ...
    pd.DataFrame(class_rows).to_csv(
        class_metrics_path, index=False, encoding="utf-8-sig"
    )
    write_markdown(
        markdown_path,
        summary_df,
    ...
    verification = verify_benchmark_outputs(
        summary_df,
        inventory_df,
        prediction_df,
        df,
        len(conventional_specs) + len(evolution_specs) + 1,
        len(df),
    )
