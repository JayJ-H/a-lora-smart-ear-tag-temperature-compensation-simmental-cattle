...
OUT_CHECK <- file.path(OUTPUT_DIR, "hierarchical_residual_oof_check.csv")
...
add_residual_features <- function(df) {
  df %>%
    mutate(
      ...
      Air20 = round(Air / 2) * 2,
      TimeInt = round(Time),
      SourceChr = as.character(ThermalRegime),
      ...
      MaySessionNum = as.integer(AcquisitionSession),
      MaySession = paste0(ThermalRegime, "_S", sprintf("%02d", MaySessionNum)),
      HourGroup = case_when(
        ThermalRegime == "stable_hot" ~ paste0("hour_", sprintf("%02d", as.integer(round(Time)))),
        Time <= 6 ~ "night",
        Time <= 11 ~ "morning_gap",
        Time <= 15 ~ "noon",
...
tight_session_eb_predict <- function(train, test) {
  global <- mean(train$Rectal)
  source_stats <- lookup_stats(train, "SourceChr")
  cow_stats <- lookup_stats(train, "CowKeyChr")
  session_stats <- lookup_stats(train, "MaySession")
  ...
  source_train <- apply_lookup(train, source_stats, "SourceChr", global, 4)
  source_test <- apply_lookup(test, source_stats, "SourceChr", global, 4)
  cow_train <- apply_lookup(train, cow_stats, "CowKeyChr", source_train, 3)
  cow_test <- apply_lookup(test, cow_stats, "CowKeyChr", source_test, 3)
  session_train <- apply_lookup(train, session_stats, "MaySession", source_train, 2)
  session_test <- apply_lookup(test, session_stats, "MaySession", source_test, 2)
  parent_train <- 0.80 * cow_train + 0.20 * session_train
  parent_test <- 0.80 * cow_test + 0.20 * session_test
  list(
    train = apply_lookup(train, cow_session_stats, "CowSession", parent_train, 0.1),
    test = apply_lookup(test, cow_session_stats, "CowSession", parent_test, 0.1)
  )
}
...
xgb_residual_predict <- function(train, test, base_train, base_test) {
  ...
  tryCatch({
    ...
    formula <- ~ Ear + Air + Time + TimeSin + TimeCos + ThermalLoad + EarAirGap +
      ...
      CowAirMean + CowAirSD + ThermalRegime + Source + CowKey + CellCoarse
    mats <- align_residual_matrices(train, test, formula)
    fit <- xgb.train(
      params = list(
...
candidate_predictions <- function(train_raw, test_raw) {
  ...
  tight_session <- tight_session_eb_predict(train, test)
  ...
  data.frame(
    ...
    tight_session = tight_session$test,
    ridge = ridge,
...
select_regime_weights <- function(train, inner_pred_mat, regime_name) {
...
run_outer_fold <- function(df, fold, outer_id) {
  ...
  inner_fold <- make_within_cow_folds(train, k = 5, seed = FOLD_SEED + 2000 + outer_id)
  candidate_cols <- c("eb_exact", "eb_medium", "eb_coarse", "global_exact", "global_medium", "global_coarse", "may_session", "may_session_hour", "tight_session", "ridge", "xgb_residual", "direct_xgb")
  ...
  for (regime in sort(unique(as.character(test$ThermalRegime)))) {
    choice <- select_regime_weights(train, inner_mat, regime)
    mask <- as.character(test$ThermalRegime) == regime
    ...
    choices[[length(choices) + 1]] <- data.frame(
      FoldID = outer_id,
      Source = as.character(test$Source[which(mask)[1]]),
...
main <- function() {
  ...
  for (outer_id in sort(unique(fold))) {
    message("hierarchical residual outer fold ", outer_id, "/", length(unique(fold)))
  ...
  checks <- data.frame(
    ...
    Detail = c(
      ...
      "Observable thermal-regime model and weight choices are selected by five-fold inner OOF evaluation within outer-training rows.",
      ...
      paste0("R2=", sprintf("%.6f", metrics$R2[1]), "; RMSE=", sprintf("%.6f", metrics$RMSE[1]), "; MAE=", sprintf("%.6f", metrics$MAE[1]), ".")
    ),
    stringsAsFactors = FALSE
  )
