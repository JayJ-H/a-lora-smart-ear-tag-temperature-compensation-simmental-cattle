...
OUTPUT = SHARED_ROOT / "输出" / "分析" / "基准模型"
INPUT = SHARED_ROOT / "数据" / "处理数据" / "th_shrc_oof_predictions.csv"


def main() -> int:
    source = pd.read_csv(INPUT)
    summary = pd.read_csv(OUTPUT / "表格" / "14_full_model_benchmark_summary.csv")
    predictions = pd.read_csv(
        OUTPUT / "表格" / "14_full_model_benchmark_predictions.csv"
    )
    inventory = pd.read_csv(
        OUTPUT / "表格" / "14_full_model_benchmark_model_inventory.csv"
    )
