...
ABLATION_INPUT = SHARED_ROOT / "数据" / "处理数据" / "th_shrc_ablation_oof_predictions.csv"
BRANCH_INPUT = SHARED_ROOT / "数据" / "处理数据" / "th_shrc_mechanism_branch_inputs.csv"
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="th_shrc_ablation_") as temp:
        ...
        if not (root / "baseline" / "表格" / "mechanism_choices.csv").is_file():
