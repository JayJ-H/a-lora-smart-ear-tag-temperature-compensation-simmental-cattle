...
COMMON_DIR = SHARED_ROOT / "分析" / "通用模块"
...
UPSTREAM_DIR = Path(__file__).resolve().parent / "上游模块"
DEFAULT_FROZEN = SHARED_ROOT / "数据" / "处理数据" / "th_shrc_oof_predictions.csv"
DEFAULT_OUTPUT = SHARED_ROOT / "输出" / "分析"
...
def run_upstream(rscript: Path, output_dir: Path) -> None:
    upstream_output = output_dir / "上游分支"
...
def reproduce(frozen_path: Path, output_dir: Path) -> dict[str, object]:
    ...
    dual = pd.read_csv(output_dir / "上游分支" / "memory_calibration_oof_predictions.csv")
    residual = pd.read_csv(output_dir / "上游分支" / "hierarchical_residual_oof_predictions.csv")
