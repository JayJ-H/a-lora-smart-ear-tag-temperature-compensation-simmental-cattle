...
IN_DATASET <- Sys.getenv("TH_SHRC_PUBLIC_INPUT", file.path(SHARED_ROOT, "数据", "处理数据", "paired_temperature_records.csv"))
...
OUT_AUDIT <- file.path(OUTPUT_DIR, "individual_session_calibration_oof_check.csv")
...
read_analysis_dataset <- function() {
  ...
  df <- df %>% transmute(RecordID, CowKey, Source, RowID, SourceRowID, Ear = EarTemperature_C, Rectal = CoreReferenceTemperature_C, Air = AmbientTemperature_C, Time = MeasurementTime_hour, SourceType, EarStatus, SourceWeight, SeqInCowSource)
  num_cols <- intersect(
    c("Ear", "Rectal", "Air", "Time", "SourceWeight", "SourceRowID", "RowID", "SeqInCowSource"),
    names(df)
  )
  ...
  df %>%
    ...
    filter(n() >= 8) %>%
    ungroup() %>%
    arrange(match(Source, c("S03", "S04", "S01")), SourceRowID)
}
...
add_batch_session_features <- function(df) {
  ...
  df <- df %>%
    ...
    group_by(Source) %>%
    arrange(SourceRowID, .by_group = TRUE) %>%
    mutate(
      SourceOrderIndex = row_number(),
      SourceN = n(),
      SourceOrderPct = ifelse(SourceN > 1, (SourceOrderIndex - 1) / (SourceN - 1), 0),
      NewSessionFlag = as.integer(row_number() == 1 | TimePeriod != lag(TimePeriod, default = first(TimePeriod))),
      SourceSessionIndex = cumsum(NewSessionFlag),
      DayIndex = pmax(1L, ceiling(SourceSessionIndex / 2))
    ) %>%
    ungroup() %>%
    mutate(
      SessionID = paste(Source, sprintf("S%02d", SourceSessionIndex), sep = "_"),
      BatchID = paste(Source, sprintf("B%02d", SourceSessionIndex), sep = "_"),
      DayID = paste(Source, sprintf("D%02d", DayIndex), sep = "_"),
      MaySourceOrderPct = ifelse(Source == "S01", SourceOrderPct, 0),
      IsMayMeasured = as.integer(Source == "S01"),
      HeatSegment = case_when(
        Source == "S01" & Air >= 30 ~ "may_hot_air30",
        Source == "S01" ~ "may_other",
        TRUE ~ Source
      )
    ) %>%
    group_by(Source, SessionID) %>%
    arrange(SourceRowID, .by_group = TRUE) %>%
    mutate(
    ...
    mutate(
      ...
      SourceRowGap1 = SourceRowID - lag(SourceRowID, 1)
    ) %>%
    ungroup() %>%
    mutate(
      ...
      SourceRowGap1 = ifelse(is.na(SourceRowGap1), 0, SourceRowGap1),
...
attach_train_target_features <- function(train, test) {
  ...
  source_tbl <- target_summary(train, c("Source"), "TrainSource")
  ...
  period_tbl <- target_summary(train, c("Source", "TimePeriod"), "TrainSourcePeriod")
...
factor_vars <- c(
  ...
  "SessionID", "BatchID", "DayID", "HeatSegment"
)
...
base_formula <- ~ Ear + I(Ear^2) + Air + I(Air^2) + Time + TimeSin + TimeCos +
  ...
  OrderEarDelta1 + OrderAirDelta1 + OrderTimeDelta1 + SourceRowGap1 +
  ...
  Source + CowKey + TimePeriod + BatchID + DayID + HeatSegment
...
gbm_formula <- Rectal ~ Ear + Air + Time + TimeSin + TimeCos + ThermalLoad + EarAirGap +
  ...
  CowGapFeature + SessionGapFeature + CowSessionGapFeature + Source + CowKey +
...
make_within_cow_folds <- function(df, k = 5, seed = 20260523) {
  set.seed(seed)
...
batch_memory_predict <- function(train, test, cfg) {
  ...
  for (i in seq_len(nrow(test))) {
    r <- test[i, ]
    pool_idx <- which(train$Source == r$Source)
    ...
    do <- abs(train$SourceRowID[pool_idx] - r$SourceRowID)
...
select_blend_choice <- function(train_meta, base_oof, mem_oof, cfgs, segment, alpha_grid = seq(0, 1, by = 0.02)) {
  seg_mask <- train_meta$HeatSegment == segment
  if (sum(seg_mask) < 8) {
    src <- unique(train_meta$Source[train_meta$HeatSegment == segment])
    if (length(src) > 0) {
      seg_mask <- train_meta$Source == src[1]
    }
  }
...
inner_oof_components <- function(train, cfgs, outer_id) {
  inner_fold <- make_within_cow_folds(train, k = 5, seed = 20260523 + 1000 + outer_id)
...
run_outer_fold <- function(df, fold, outer_id, cfgs) {
  ...
  train_meta <- train %>% transmute(Rectal = Rectal, Source = Source, HeatSegment = HeatSegment)
  for (seg in sort(unique(test$HeatSegment))) {
    choice <- select_blend_choice(train_meta, inner$base, inner$memory, cfgs, seg)
    mask <- test$HeatSegment == seg
    ...
    nearest_distance[mask] <- test_mem_audit[[ci]]$nearest_distance[mask]
    nearest_row_id[mask] <- test_mem_audit[[ci]]$nearest_row_id[mask]
    nearest_source[mask] <- test_mem_audit[[ci]]$nearest_source[mask]
    nearest_session[mask] <- test_mem_audit[[ci]]$nearest_session[mask]
    exact_match[mask] <- test_mem_audit[[ci]]$exact_observable_match[mask]
