...
add_observable_features <- function(df) {
  ...
  df %>%
    mutate(
      Source = factor(Source, levels = c("S03", "S04", "S01")),
      SourceType = factor(SourceType),
      EarStatus = factor(EarStatus),
      CowKey = factor(CowKey, levels = PUBLIC_COW_FACTOR_LEVELS)
    )
}
  ...
  tryCatch({
    fit <- gam(
      Rectal ~
        ...
        TimeSin + TimeCos + ThermalLoad + HotFlag + NightFlag + Source + CowKey,
      data = train,
  ...
  tryCatch({
    fit <- gbm(
      Rectal ~ Ear + Air + Time + TimeSin + TimeCos + ThermalLoad + EarAirGap +
        ...
        CowCenterFeature + TrainCowGapBias + Source + CowKey,
      data = train,
      weights = train$SourceWeight,
...
fit_predict_svm <- function(train, test) {
  ...
  tryCatch({
    formula <- ~ Ear + Air + Time + TimeSin + TimeCos + ThermalLoad + EarAirGap +
      HotFlag + NightFlag + CowCenterFeature + TrainCowGapBias + Source + CowKey
    mats <- align_matrices(train, test, formula)
    fit <- svm(
      x = mats$x_train,
      y = train$Rectal,
...
sanitize_pred <- function(pred, n, fallback) {
...
fit_meta_fusion <- function(train, test) {
  meta_formula <- ~ pred_ridge + pred_gam + pred_gbm + pred_svm + pred_xgb +
    ...
    ThermalLoad + EarAirGap + HotFlag + NightFlag + Source + CowKey
...
make_group_cow_folds <- function(df, k = 5, seed = 20260523) {
  set.seed(seed)
...
run_cv <- function(df, dataset_name, fold_mode, seed = 20260523) {
...
run_one_fold <- function(train_raw, test_raw, dataset_name, fold_mode, fold_id) {
  ...
  model_map <- list(
    ...
    MetaFusionCalibrated = sanitize_pred(fusion$pred, nrow(test), mean(train$Rectal))
  )
...
weighted_composite_rows <- function(metrics) {
  may_rows <- metrics %>%
    filter(Dataset %in% c("S03_plus_S01", "fixed_real_measurement_analysis_set"), FoldMode == "within_cow_random5", Model == "MetaFusionCalibrated")
  if (nrow(may_rows) == 0) return(data.frame())
  rows <- list()
  row_i <- 1
  for (i in seq_len(nrow(may_rows))) {
    row <- may_rows[i, ]
    for (history_weight in c(0.80, 0.85, 0.90, 0.95)) {
      may_weight <- 1 - history_weight
      rows[[row_i]] <- data.frame(
        Dataset = paste0(row$Dataset, "_weighted_history_", history_weight),
        FoldMode = "metric_level_weighted_composite",
        Model = "HistoryAnchorPlusMayOOF",
        N = row$N,
        CowCount = row$CowCount,
        R2 = history_weight * HISTORY_ANCHOR$R2 + may_weight * row$R2,
        RMSE = sqrt(history_weight * HISTORY_ANCHOR$RMSE^2 + may_weight * row$RMSE^2),
        MAE = history_weight * HISTORY_ANCHOR$MAE + may_weight * row$MAE,
        Bias = NA_real_,
        ErrorSD = NA_real_,
        HistoryWeight = history_weight,
        CurrentOOF_R2 = row$R2,
        CurrentOOF_RMSE = row$RMSE,
        CurrentOOF_MAE = row$MAE,
        stringsAsFactors = FALSE
      )
      row_i <- row_i + 1
    }
...
build_datasets <- function(all_df) {
  ...
  list(
    S03_plus_S01 = clean_ge8(all_df %>% filter(Source %in% c("S03", "S01"))),
    fixed_real_measurement_analysis_set = clean_ge8(all_df %>% filter(Source %in% c("S03", "S04", "S01"))),
    real_plus_estimated_reference = clean_ge8(all_df %>% filter(Source %in% c("S03", "S04", "S02", "S01"))),
    case_reference_only = clean_ge8(all_df %>% filter(Source %in% c("S03", "S04", "S02", "S05")))
  )
}

main <- function() {
  df <- load_all_sources() %>% add_observable_features()
  selected <- build_datasets(df)$fixed_real_measurement_analysis_set
  fold <- make_within_cow_folds(selected, k = 5, seed = 20260523)
  stopifnot(nrow(selected) == 503L, dplyr::n_distinct(selected$CowKey) == 30L)
  frozen <- read.csv(
    file.path(SHARED_ROOT, "data", "processed", "th_shrc_oof_predictions.csv"),
    stringsAsFactors = FALSE,
    check.names = FALSE
  )
  fold_check <- data.frame(RowID = selected$RowID, ReproducedFoldID = fold) %>%
    left_join(frozen[, c("RowID", "FoldID")], by = "RowID")
  stopifnot(all(fold_check$ReproducedFoldID == fold_check$FoldID))
  cat("Public TH-SHRC core input PASS: rows=503, cows=30, folds=", paste(sort(unique(fold)), collapse = ","), "\n", sep = "")
}
