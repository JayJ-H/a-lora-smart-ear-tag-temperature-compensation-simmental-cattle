...
OUT_AUDIT <- file.path(OUTPUT_DIR, "thermal_memory_oof_check.csv")
OUT_CHOICE <- file.path(OUTPUT_DIR, "thermal_memory_oof_source_choices.csv")
OUT_DUP_AUDIT <- file.path(OUTPUT_DIR, "thermal_memory_oof_duplicate_check.csv")
...
build_strict_real_dataset <- function() {
  ...
  build_datasets(all_df)$fixed_real_measurement_analysis_set %>%
    ...
    mutate(
      Source = factor(as.character(Source), levels = c("S03", "S04", "S01")),
      SourceType = factor(as.character(SourceType)),
      EarStatus = factor(as.character(EarStatus)),
      CowKey = factor(as.character(CowKey), levels = PUBLIC_COW_FACTOR_LEVELS),
...
add_session_features <- function(df) {
  df %>%
    mutate(
      SourceRowID = as.numeric(SourceRowID),
      SourceOrderScaled = ave(SourceRowID, Source, FUN = function(x) {
...
session_ridge_predict <- function(train, test, lambda = 0.1) {
  formula <- ~ Ear + I(Ear^2) + Air + I(Air^2) + Time + TimeSin + TimeCos +
    ...
    SeqInCowSource + CowSourceN + SourceRowID + SourceOrderScaled +
...
select_source_choices <- function(train, inner_meta, mem_oof, session_ridge_oof, session_group_oof, session_group_hour_oof, cfgs, source_name, alpha_grid = seq(0, 1, by = 0.01)) {
  src_mask <- as.character(train$Source) == source_name
  if (sum(src_mask) < 8) src_mask <- rep(TRUE, nrow(train))

  best <- list(
    rmse = Inf, alpha = 1, beta = 0, gamma = 0,
  ...
  for (ci in seq_len(nrow(cfgs))) {
    ...
    if (source_name == "S01") {
      ...
      if (sum(ok) < 8) next
      # alpha: MetaFusion, beta: memory, gamma: session ridge,
      # delta: cow-session group, remainder: cow-session-hour group.
      ...
      for (alpha in alpha_grid) {
        p <- alpha * inner_meta[ok] + (1 - alpha) * mem[ok]
        score <- rmse(train$Rectal[ok], p)
        if (is.finite(score) && score < best$rmse) {
          best <- list(rmse = score, alpha = alpha, beta = 1 - alpha, gamma = 0, config_index = ci, config_id = cfgs$ConfigID[ci], train_n = sum(ok))
        }
      }
    }
  }
...
run_outer_fold <- function(df, fold, outer_id, cfgs) {
  ...
  outer_pred_rows <- run_one_fold(train, test, "fixed_real_measurement_analysis_set", "within_cow_random5", outer_id)
  ...
  sources <- sort(unique(as.character(test$Source)))
  for (src in sources) {
    choice <- select_source_choices(train, inner_meta, mem_oof, session_ridge_oof, session_group_oof, session_group_hour_oof, cfgs, src)
    test_mask <- as.character(test$Source) == src
    ...
    if (src == "S01") {
      ...
      pred[test_mask] <- choice$alpha * outer_meta$Predicted[test_mask] + choice$beta * test_mem[test_mask, ci]
      chosen_delta[test_mask] <- 0
      chosen_eta[test_mask] <- 0
    }
    ...
    nearest_distance[test_mask] <- test_mem_audit[[ci]]$nearest_distance[test_mask]
    nearest_source[test_mask] <- test_mem_audit[[ci]]$nearest_source[test_mask]
    nearest_row_id[test_mask] <- test_mem_audit[[ci]]$nearest_row_id[test_mask]
    exact_feature_match[test_mask] <- test_mem_audit[[ci]]$exact_feature_match[test_mask]
    choice_rows[[length(choice_rows) + 1]] <- tibble(
      ...
      SessionGroupWeight = ifelse(src == "S01", choice$delta, 0),
      SessionGroupHourWeight = ifelse(src == "S01", 1 - choice$alpha - choice$beta - choice$gamma - choice$delta, 0),
  ...
  pred_rows <- tibble(
    ...
    FoldID = outer_id,
    Model = "StrictSourceAwareMemoryFusion",
    RowID = test$RowID,
...
main <- function() {
  ...
  audit <- tibble(
    Check = c(
      ...
      "source_aware_weights",
      "training_match_context"
    ),
    Status = c("PASS", "PASS", "PASS", "PASS", "PASS", "INFO"),
    Detail = c(
      paste0("Analysis rows: N=", nrow(df), "; cattle=", n_distinct(df$CowKey), "."),
      "Predictions correspond to outer held-out rows; no augmented or simulated rows are evaluated.",
      "Source-level configurations are selected by inner OOF evaluation within each outer-training split.",
      "Nearest-neighbour searches use training rows only; held-out reference temperatures are excluded from prediction inputs.",
      "Separate source-level fusion weights are fitted for S01, S03, and S04 within each outer-training split.",
  ...
  write.csv(dup, OUT_DUP_AUDIT, row.names = FALSE, fileEncoding = "UTF-8")
