...
def main() -> int:
    status = read_rows(DATA / "元数据/temperature_candidate_row_status.csv")
    summary = read_rows(DATA / "元数据/temperature_selection_summary.csv")
    raw = read_rows(DATA / "匿名原始数据/paired_temperature_raw_anonymized.csv")
    processed = read_rows(DATA / "处理数据/paired_temperature_records.csv")
    oof = read_rows(DATA / "处理数据/th_shrc_oof_predictions.csv")
    ...
    for name, rows in {"raw": raw, "处理数据": processed, "oof": oof}.items():
