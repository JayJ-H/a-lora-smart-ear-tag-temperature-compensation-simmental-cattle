...
DEFAULT_DATA = SHARED / "数据"
DEFAULT_OUTPUT = SHARED / "输出"
...
def verify_manifest(data_dir: Path) -> None:
    group = "provenance"
    manifest_path = data_dir / "元数据/result_source_manifest.csv"
...
def verify_main_oof(data_dir: Path) -> dict[str, dict[str, str]]:
    group = "main_th_shrc"
    rows = read_rows(data_dir / "处理数据/th_shrc_oof_predictions.csv")
    ...
    add_check(
        ...
        "oof_design",
        shape_ok,
        "503 条唯一记录、30 个 CowKey，以及 5 个牛内随机 OOF 折。",
    ...
    add_check(
        ...
        difference <= 1e-12,
        "根据全部 OOF 预测重新计算 R2、RMSE、MAE 和 0.5 °C 覆盖率。",
...
def verify_benchmark(data_dir: Path, oof: dict[str, dict[str, str]]) -> None:
    group = "基准模型"
    rows = read_rows(data_dir / "处理数据/model_benchmark_oof_predictions.csv")
    summaries = read_rows(data_dir / "处理数据/model_benchmark_metrics.csv")
    ...
    add_check(
        ...
        identity_failures == 0,
        "每个模型使用与官方 OOF 表相同的 RowID 和 Actual 值。",
        identity_failures,
        0,
    )
    add_check(
...
def verify_ablation(data_dir: Path, oof: dict[str, dict[str, str]]) -> None:
    group = "消融"
    rows = read_rows(data_dir / "处理数据/th_shrc_ablation_oof_predictions.csv")
    summaries = read_rows(data_dir / "处理数据/th_shrc_ablation_metrics.csv")
    bootstrap = read_rows(data_dir / "处理数据/th_shrc_ablation_bootstrap.csv")
    by_row = {row["RowID"]: row for row in rows}
...
def verify_shap(data_dir: Path, oof: dict[str, dict[str, str]]) -> None:
    group = "shap"
    rows = read_rows(data_dir / "处理数据/th_shrc_shap_values.csv")
    summaries = read_rows(data_dir / "处理数据/th_shrc_shap_summary.csv")
    by_row = {row["RowID"]: row for row in rows}
    ...
    add_check(
        ...
        "4 个展示的 SHAP 分组摘要及其排序均由 503 条记录重新计算。",
    ...
    add_check(
        ...
        "技术基线加 4 个条件 SHAP 分组可重建条件全流程预测值。",
        additivity_difference,
        0.0,
        "1e-12",
    )
    ...
    add_check(
        ...
        "条件 SHAP 表重建条件全流程预测值；报告指标由 OOF 预测重新计算。",
...
def verify_lora(data_dir: Path) -> None:
    group = "lora"
    rows = read_rows(data_dir / "处理数据/lora_packet_level_records.csv")
    ...
    add_check(
        ...
        "packet_test_design",
        design_ok,
        "数据包表包含 5 个距离、8 个方向，每个组合 100 个数据包。",
    ...
    add_check(
        ...
        signal_ok,
        "Received 与丢包标志互补；249 个丢包的 91 哨兵值仅保存在 SourceSentinelValue 中，不进入 RSSI/SNR。",
    )
...
def write_reports(output_dir: Path) -> None:
    ...
    lines = [
        "# 统一结果验证",
        "",
        f"总体：**{payload['overall_status']}**（{passed}/{len(CHECKS)} 项检查通过）",
        "",
        "| 分组 | 检查项 | 状态 | 说明 |",
        "|---|---|---:|---|",
    ]
    ...
    lines.extend(["", "TH-SHRC、基准模型、消融、SHAP 和 LoRa 检查均使用逐行输入数据。", ""])
...
def main() -> int:
    ...
    run_group("基准模型", lambda: verify_benchmark(data_dir, oof))
    run_group("消融", lambda: verify_ablation(data_dir, oof))
