...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-upstream",
        action="store_true",
        help="Reuse TH-SHRC upstream files already present below OUTPUT_DIR/分析/upstream.",
    )
    parser.add_argument(
        "--shap-mode",
...
def build_stages(args: argparse.Namespace, rscript: Path) -> list[tuple[str, list[str]]]:
    ...
    analysis_output = output_root / "分析"
    benchmark_output = analysis_output / "基准模型"
    interpretability_output = analysis_output / "可解释性"
    python = sys.executable

    full_pipeline = [
        python,
        "分析/TH-SHRC/reproduce_full_pipeline.py",
        "--output-dir",
        str(analysis_output),
        "--rscript",
        str(rscript),
    ...
    stages: list[tuple[str, list[str]]] = [
        (
            ...
            [
                python,
                "脚本/run_temperature_analysis.py",
                "--output-dir",
                str(output_root),
                "--verify-docs",
            ],
        ...
        (
            ...
            [
                python,
                "分析/消融/run_ablation.py",
                "--output-dir",
                str(analysis_output / "消融"),
                "--candidate-input",
                str(analysis_output / "上游分支" / "hierarchical_residual_oof_predictions.csv"),
            ],
        ),
        (
            "native_stack_shap",
            [
                python,
                "分析/可解释性/run_native_stack_shap.py",
                "--output-dir",
                str(interpretability_output / "原生堆叠SHAP"),
            ],
        ),
        (
            ...
            [
                python,
                "分析/可解释性/verify_conditional_shap.py",
                "--output-dir",
                str(interpretability_output / "条件SHAP"),
            ],
        ),
    ]
    ...
    if args.shap_mode != "verify":
        shap_command = [
            str(rscript),
            "分析/可解释性/run_full_pipeline_shap.R",
            "--output-dir",
            str(interpretability_output / "完整流程SHAP"),
        ]
    ...
    stages.extend(
        [
            (
                ...
                [
                    str(rscript),
                    "分析/可解释性/plot_publication_figures.R",
                    "--output-dir",
                    str(interpretability_output / "图件"),
                ],
            ),
            (
