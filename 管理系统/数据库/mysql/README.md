...
```bash
docker compose -f 数据库/mysql/docker-compose.yml up -d
```

首次启动会自动执行：

- `数据库/mysql/init/001_init_schema.sql`
- `数据库/mysql/init/002_event_sync_triggers.sql`
...
```bash
mysql -uroot -p < 数据库/mysql/init/001_init_schema.sql
```
...
第一阶段收敛口径见 `数据库/mysql/DATA_DICTIONARY.md`。
...
- `数据库/mysql/migrations/030_model_convergence_dry_run.sql` 只包含只读检测和迁移建议，不应在生产库直接执行写入迁移。
