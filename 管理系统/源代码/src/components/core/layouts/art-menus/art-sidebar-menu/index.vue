...
<template>
  <div
    ...
    :class="{ 'no-border': menuList.length === 0, 'is-open': menuOpen, 'is-closed': !menuOpen }"
    :style="{ '--sidebar-open-width': menuopenwidth }"
  >
    <!-- 双列菜单（左侧） -->
    <div
      v-if="isDualMenu"
    ...
    <div
      ...
      class="menu-left"
      :class="`menu-left-${getMenuTheme.theme} menu-left-${!menuOpen ? 'close' : 'open'} console-rail`"
...
<script setup lang="ts">
  ...
  const EXPANDED_MENU_MIN_WIDTH = 240
  ...
  const menuopenwidth = computed(() => {
    const width = Number.parseInt(getMenuOpenWidth.value, 10)
    return `${Number.isFinite(width) ? Math.max(width, EXPANDED_MENU_MIN_WIDTH) : EXPANDED_MENU_MIN_WIDTH}px`
  })
