...
export function useChart(options: UseChartOptions = {}) {
              ...
              if (!isDestroyed && pendingOptions) {
                try {
                  ...
                  if (!chart) {
                    ...
                    const reused = echarts.getInstanceByDom(dom)
                    chart = reused && !reused.isDisposed() ? reused : echarts.init(dom)
                  }
  ...
  const performChartInit = (options: EChartsOption) => {
    if (!chart && chartRef.value && !isDestroyed) {
      ...
      chart = reused && !reused.isDisposed() ? reused : echarts.init(chartRef.value)
      // 图表创建后立即设置监听器
      setupMenuWatchers()
      setupThemeWatcher()
    }
