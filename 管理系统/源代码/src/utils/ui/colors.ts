...
export function handleElementThemeColor(theme: string, isDark: boolean = false): void {
  ...
  syncFluentThemeColor(theme, isDark)
...
/**
 * Keep the Fluent 2 token layer in sync with the selected Element Plus primary color.
 */
export function syncFluentThemeColor(theme: string, isDark: boolean = false): void {
  const [red, green, blue] = hexToRgb(theme)
  const elStyle = document.documentElement.style
  const softColor = getLightColor(theme, 0.88)
  const strongColor = getDarkColor(theme, 0.28)

  elStyle.setProperty('--fluent-primary', theme)
  elStyle.setProperty('--fluent-primary-rgb', `${red} ${green} ${blue}`)
  elStyle.setProperty('--fluent-primary-soft', softColor)
  elStyle.setProperty('--fluent-primary-strong', strongColor)

  if (isDark) {
    elStyle.setProperty('--fluent-border', 'rgb(255 255 255 / 12%)')
    elStyle.setProperty('--fluent-border-strong', `rgb(${red} ${green} ${blue} / 32%)`)
    elStyle.setProperty('--art-hover-color', `rgb(${red} ${green} ${blue} / 14%)`)
    elStyle.setProperty('--art-active-color', `rgb(${red} ${green} ${blue} / 18%)`)
    elStyle.setProperty('--art-el-active-color', `rgb(${red} ${green} ${blue} / 16%)`)
    elStyle.setProperty('--fluent-focus-ring', `0 0 0 3px rgb(${red} ${green} ${blue} / 18%)`)
    return
  }

  elStyle.setProperty('--fluent-border', `rgb(${red} ${green} ${blue} / 22%)`)
  elStyle.setProperty('--fluent-border-strong', `rgb(${red} ${green} ${blue} / 38%)`)
  elStyle.setProperty('--art-hover-color', `rgb(${red} ${green} ${blue} / 9%)`)
  elStyle.setProperty('--art-active-color', `rgb(${red} ${green} ${blue} / 13%)`)
  elStyle.setProperty('--art-el-active-color', `rgb(${red} ${green} ${blue} / 11%)`)
  elStyle.setProperty('--default-bg-color', softColor)
  elStyle.setProperty('--fluent-focus-ring', `0 0 0 3px rgb(${red} ${green} ${blue} / 12%)`)
}
