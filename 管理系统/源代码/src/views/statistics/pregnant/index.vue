<template>
  <div class="statistics-page p-5">
    <div class="flex items-center justify-between mb-5">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">预产牛统计</h1>
      </div>

      <div class="flex items-center gap-4">
        <div class="text-sm text-gray-600 dark:text-gray-300"> 共{{ totalCows }}头预产牛只 </div>
        <ElButton type="primary" @click="loadPregnantCows" :loading="loading">
    ...
    <div v-else class="text-center py-16">
      ...
      <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">暂无预产牛只</h3>
    </div>
...
<script setup lang="ts">
  ...
  const pregnantCows = ref<CowBasic[]>([])
  ...
  const getCowSensorSeries = (cowId: string) => {
    const cow = pregnantCows.value.find((item) => String(item.id) === String(cowId))
  ...
  // 获取预产牛只列表
  const loadPregnantCows = async () => {
    loading.value = true
    try {
      const context = await buildUnifiedDataContext()
      let allCows = (context.cows || []) as CowBasic[]
      // 绛涢€夐浜х墰鍙苟鎸夌墰鍙锋帓搴?
      const pregnant = allCows
        .filter(
          (cow: any) =>
            cow.pregnancy === true || ['妊娠', '预产'].includes(normalizeStatus(cow.status))
        )
        .sort(sortByCowNumber)

      pregnantCows.value = pregnant
      totalCows.value = pregnant.length

      // 鍔犺浇浼犳劅鍣ㄦ暟鎹?
      void loadSensorData(pregnant)
    } catch (error) {
      console.error('加载预产牛只失败:', error)
    } finally {
      loading.value = false
    }
  }
  ...
  } = useLazyGridRenderWindow(pregnantCows, {
    rowCount: 2,
    minItemWidth: 280,
    gap: 24,
    fallbackColumns: 4,
  ...
  // 监听 pregnantCows 变化，重新渲染图表
  defineOptions({ name: 'PregnantCows' })
</script>
