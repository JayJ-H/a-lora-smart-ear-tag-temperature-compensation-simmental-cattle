<template>
  <div class="statistics-page p-5">
    <div class="flex items-center justify-between mb-5">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">发情牛统计</h1>
      </div>

      <div class="flex items-center gap-4">
        <div class="text-sm text-gray-600 dark:text-gray-300"> 共{{ totalCows }}头发情牛只 </div>
        <ElButton type="primary" @click="loadHeatCows" :loading="loading">
    ...
    <div v-else class="text-center py-16">
      ...
      <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">暂无发情牛只</h3>
    </div>
        ...
        <div class="statistics-detail-block">
          ...
          <div class="statistics-detail-grid grid gap-4">
            <div class="bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-lg">
              <div class="text-sm text-yellow-800 dark:text-yellow-200 font-medium">牛号</div>
...
<script setup lang="ts">
  ...
  const getCowSensorSeries = (cowId: string) => {
    const cow = heatCows.value.find((item) => String(item.id) === String(cowId))
  ...
  // 获取发情牛只列表
  const loadHeatCows = async () => {
    loading.value = true
    try {
      const context = await buildUnifiedDataContext()
      let allCows = (context.cows || []) as CowBasic[]
      const heat = allCows
        .filter((cow: any) => normalizeStatus(cow.status) === '发情')
        .sort(sortByCowNumber)

      heatCows.value = heat
      totalCows.value = heat.length

      // 鍔犺浇浼犳劅鍣ㄦ暟鎹?
      void loadSensorData(heat)
    } catch (error) {
      console.error('加载发情牛只失败:', error)
    } finally {
      loading.value = false
    }
  }
  ...
  } = useLazyGridRenderWindow(heatCows, {
    rowCount: 2,
    minItemWidth: 280,
    gap: 24,
    fallbackColumns: 4,
  ...
  // 监听 heatCows 变化，重新渲染图表
  defineOptions({ name: 'HeatCows' })
</script>
