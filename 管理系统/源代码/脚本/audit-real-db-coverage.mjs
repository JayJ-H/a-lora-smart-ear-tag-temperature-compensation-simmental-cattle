...
const SCAN_TARGETS = [
  ...
  path.join(projectRoot, '脚本', 'mysql-backend-server.mjs')
]
...
function categorizeBlocker(item) {
  ...
  if (haystack.includes('sensor') || haystack.includes('device') || haystack.includes('硬件') || haystack.includes('传感')) return 'sensor'
...
async function scanBackendRisks() {
  ...
  return {
    file: '脚本/mysql-backend-server.mjs',
...
async function getHttpSamples() {
  ...
  try {
    ...
    const [automation, protocol, diagnostics] = await Promise.all([
      ...
      postJson('/api/cow/硬件/testProtocolConnection', { args: ['audit-missing-protocol'] }, authHeaders),
      postJson('/api/cow/硬件/runSystemDiagnostics', { args: [] }, authHeaders)
    ])
...
function buildProductionBlockers({
  ...
  if (backendRisks.mathRandomCount > 0) {
    blockers.push({
      ...
      detail: `脚本/mysql-backend-server.mjs still has ${backendRisks.mathRandomCount} Math.random hits`
    })
  }
  if (
...
async function main() {
  ...
  const report = {
    ...
    staticScan: {
      scope: ['src/api/cow.ts', 'src/views', '脚本/mysql-backend-server.mjs'],
      files: staticFiles
    },
    frontendMockIsolation,
    backendRisks,
