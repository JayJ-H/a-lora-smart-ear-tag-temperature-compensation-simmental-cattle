...
dotenv.config({ path: path.join(projectRoot, '运维/生产配置/.env.prod') })
...
async function main() {
  ...
  if (authMode === 'strict' && (!adminUser || !adminPassword)) {
    throw new Error(
      'Production backend is in strict auth mode but no admin credentials were provided. Set PRODUCTION_ADMIN_USER/PRODUCTION_ADMIN_PASSWORD or configure 运维/生产配置/.env.prod.'
    )
  }
