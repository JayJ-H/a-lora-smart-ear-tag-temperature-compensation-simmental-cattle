...
async function showCleanupWarning() {
  createCard('安全警告', [
    `${fmt.warning('此操作将永久删除以下预置内容，且无法恢复！')}`,
  ...
  const cleanupItems = [
    ...
    },
    {
      icon: icons.code,
      name: '路由模块文件',
      desc: '删除预置路由模块，只保留核心模块（dashboard、system、result、exception）',
      color: theme.primary
    },
    {
      icon: icons.link,
      name: '路由别名',
      desc: '重写routesAlias.ts，移除预置路由别名',
      color: theme.info
    },
    {
      icon: icons.data,
      name: 'Mock数据',
      desc: '预置的JSON数据、文章列表、评论数据等',
      color: theme.success
    },
    {
      icon: icons.globe,
      name: '多语言文件',
      desc: '清理中英文语言包中的预置菜单项',
      color: theme.warning
    },
...
async function main() {
  ...
  // 开始清理过程
  console.log(`  ${fmt.badge('步骤 1/6', theme.bgBlue)} ${fmt.title('删除预置文件')}`)
  console.log()
