...
const backend = fs.readFileSync('脚本/mysql-backend-server.mjs', 'utf8')
...
const dockerfile = fs.readFileSync('运维/生产配置/Dockerfile.api', 'utf8')
assert.ok(dockerfile.includes('COPY 脚本/th-shrc-runtime.mjs'))
assert.ok(dockerfile.includes('COPY 脚本/assets/th-shrc'))

const compose = fs.readFileSync('运维/生产配置/docker-compose.prod.yml', 'utf8')
assert.ok(compose.includes('TH_SHRC_ENABLED: ${TH_SHRC_ENABLED:-true}'))

const liveCheck = fs.readFileSync('脚本/check-th-shrc-live.mjs', 'utf8')
for (const requiredText of [
