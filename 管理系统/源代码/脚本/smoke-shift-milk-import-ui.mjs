...
const screenshotDir = path.resolve('artifacts/shift-milk-import-ui')
...
const cases = [
  {
    ...
    file: '01_个体档案_animal-profile_先导入.xlsx',
    expectedText: ['265 行可入库', '预检通过']
  },
  {
    id: 'milk-measurement',
    template: 'milk-measurement',
    file: '03_泌乳奶厅测量_milk-measurement.xlsx',
    expectedText: ['未匹配到现有牛只'],
    expectedMode: 'blocked-until-profile-import'
  }
]
...
async function runCase(page, item) {
  ...
  await page.waitForSelector('.upload-box', { timeout: 15000 })
  ...
  const missing = item.expectedText.filter((token) => !text.includes(token))
  const expectedBlocked = item.expectedMode === 'blocked-until-profile-import'
  return {
    id: item.id,
    ...
    ok:
      missing.length === 0 &&
      (expectedBlocked || !/表格预检失败|提交失败|待处理\\d+ 行需修正/.test(text)),
    expectedMode: item.expectedMode || 'dry-run-pass',
    missing,
