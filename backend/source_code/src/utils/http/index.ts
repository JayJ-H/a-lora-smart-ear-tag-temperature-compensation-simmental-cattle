  ...
  (error) => {
    if (error.response?.status === ApiStatus.unauthorized) {
      if (isLoginRequest(error.config?.url)) {
        const message = error.response?.data?.msg || $t('httpMsg.unauthorized')
        throw createHttpError(message, ApiStatus.unauthorized)
      }
...
async function request<T = any>(config: ExtendedAxiosRequestConfig): Promise<T> {
  ...
  try {
    ...
    if (error instanceof HttpError && error.code !== ApiStatus.unauthorized) {
      ...
      const showLog = config.showErrorLog !== false
      showError(error, showMsg, showLog)
    }
    return Promise.reject(error)
  }
}
