  ...
  >
    <template #metrics>
      <section class="fc-metric-grid">
        <FcMetricTile
          label="候选母牛"
          :value="femaleRows.length"
          note="进入当前基线指数排行的母牛数量"
          icon="ri:user-heart-line"
        />
        <FcMetricTile
          label="高泌乳候选"
          :value="highMilkRows.length"
          note="平均泌乳量 >= 9kg"
          icon="ri:drop-line"
          tone="teal"
        />
    ...
    <FcPanel title="算法与追溯摘要">
      <div class="trace-grid">
        ...
        <section class="trace-block">
          <span>输入快照</span>
          <strong
            >{{ traceSummary.inputSnapshot.candidateSet.count }} 头母牛 /
            {{ traceSummary.inputSnapshot.mateCandidateSet.count }} 头公牛</strong
          >
          <p
            >输出
        ...
        <section class="ranking-grid">
          ...
          >
            ...
            <div class="score-grid">
              <span
                >泌乳 <b>{{ item.row.milkScore }}</b></span
              >
              <span
              ...
              >
              <span
                >均奶 <b>{{ item.row.averageMilk.toFixed(1) }}kg</b></span
              >
            </div>
    ...
    <ElDialog v-model="candidateDetailVisible" title="候选母牛详情" width="920px">
...
<script setup lang="ts">
  ...
  const primaryTrait = ref<TraitKey>('milk_yield')
  const primaryDirection = ref<SortDirection>('desc')
  const secondaryTrait = ref<TraitKey>('genomicScore')
  ...
  const traceSummary = computed(() =>
    buildCandidateTraceSummary({
      ...
      candidateRows: femaleRows.value,
      mateRows: bullRows.value,
  ...
  const highMilkRows = computed(() => femaleRows.value.filter((row) => row.averageMilk >= 9))
      ...
      metrics: [
        {
          ...
          note: '由泌乳、组学证据分、系谱完整度、健康和候选标签按基线权重排序。'
        },
        {
        ...
        {
          label: '泌乳基线分',
          value: item.row.milkScore,
          note: `均奶 ${item.row.averageMilk.toFixed(1)} kg，来自奶厅和表型记录。`
        },
        {
          label: '组学证据分',
  ...
  const announceSelection = async () => {
    try {
      ...
      ElMessage.success(`已生成 ${rows.length} 头候选母牛基线指数排行榜，运行记录 ${runId}`)
    } catch (error) {
      ElMessage.error('母牛榜单运行记录写入失败')
      console.error(error)
    }
  }
  ...
  const loadData = async () => {
    ...
    if (!traitOptions.value.some((item) => item.value === primaryTrait.value))
      primaryTrait.value = traitOptions.value[0]?.value || 'milk_yield'
    if (!traitOptions.value.some((item) => item.value === secondaryTrait.value))
      secondaryTrait.value =
        traitOptions.value.find((item) => item.value === 'genomicScore')?.value ||
