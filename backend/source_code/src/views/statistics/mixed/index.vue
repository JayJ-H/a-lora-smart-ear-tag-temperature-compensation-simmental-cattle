<template>
  <div class="statistics-page p-5">
    <div class="flex items-center justify-between mb-5">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">混群牛统计</h1>
      </div>

      <div class="flex items-center gap-4">
        <div class="text-sm text-gray-600 dark:text-gray-300"> 共{{ totalCows }}头混群牛只 </div>
        <ElButton type="primary" @click="loadMixedCows" :loading="loading">
          ...
          <div class="mt-3 pt-3 border-t border-white/30">
            ...
            <div class="grid grid-cols-2 gap-3 mb-3">
              ...
              <div class="statistics-soft-cell px-3 py-2">
                ...
                <div class="text-sm font-semibold text-gray-900 dark:text-white">{{
                  formatSensorSteps(getLatestSensorData(cow.id))
                }}</div>
              </div>
            </div>
          </div>
    ...
    <div v-else class="text-center py-16">
      ...
      <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">暂无混群牛只</h3>
    </div>
...
<script setup lang="ts">
  ...
  const getCowSensorSeries = (cowId: string) => {
    const cow = mixedCows.value.find((item) => String(item.id) === String(cowId))
  ...
  const loadMixedCows = async () => {
    ...
    try {
      ...
      // 绛涢€夋贩缇ょ墰鍙苟鎸夌墰鍙锋帓搴?
      const mixed = allCows
        .filter((cow: any) => cow.mixing === true || normalizeStatus(cow.status) === '混群')
        .sort(sortByCowNumber)

      mixedCows.value = mixed
      totalCows.value = mixed.length

      // 鍔犺浇浼犳劅鍣ㄦ暟鎹?
      void loadSensorData(mixed)
    } catch (error) {
      console.error('加载混群牛只失败:', error)
    } finally {
      loading.value = false
    }
  }
  ...
  const formatSensorSteps = (data: ExtendedSensorData | null) => {
    const value = Number((data as any)?.steps)
  ...
  } = useLazyGridRenderWindow(mixedCows, {
    rowCount: 2,
    minItemWidth: 280,
    gap: 24,
    fallbackColumns: 4,
  ...
  // 监听 mixedCows 变化，重新渲染图表
  defineOptions({ name: 'MixedCows' })
</script>
