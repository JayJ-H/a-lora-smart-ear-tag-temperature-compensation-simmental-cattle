...
<template>
  ...
  >
    ...
    <!-- 提示文本 -->
    <div class="dv_text" :style="textStyle" ref="messageRef" aria-live="polite">
...
<script setup lang="ts">
  ...
  const getMaxPosition = (): number => Math.max(getNumericWidth() - props.height, 0)

  const progressPercent = computed(() => {
    const maxPosition = getMaxPosition()
    return maxPosition === 0 ? 0 : Math.round((state.position / maxPosition) * 100)
  })

  const progressValueText = computed(() => {
    return props.value ? props.successText : `${progressPercent.value}%`
  })
  ...
  const setPosition = (position: number) => {
    ...
    handler.value.style.left = `${nextPosition}px`
    progressBar.value.style.width =
      nextPosition === 0 ? '0' : `${nextPosition + props.height / 2}px`
  }
  ...
  const dragMoving = (e: MouseEvent | TouchEvent) => {
    if (state.isMoving && !props.value) {
      ...
      // 计算当前位置
      const pageX = e instanceof TouchEvent ? e.touches[0].pageX : e.pageX
      const nextPosition = pageX - state.x
  ...
  const dragFinish = (e: MouseEvent | TouchEvent) => {
    if (state.isMoving && !props.value) {
      const maxPosition = getMaxPosition()
      // 计算最终位置
      const pageX = e instanceof TouchEvent ? e.changedTouches[0].pageX : e.pageX
      const nextPosition = pageX - state.x
  ...
  const handleKeydown = (e: KeyboardEvent) => {
    ...
    const maxPosition = getMaxPosition()
    const step = Math.max(maxPosition / 10, 1)
    ...
    switch (e.key) {
      ...
      case 'ArrowUp':
        e.preventDefault()
        e.stopPropagation()
        setPosition(state.position + step)
        if (state.position >= maxPosition) passVerify()
        break
      case 'ArrowLeft':
      case 'ArrowDown':
        e.preventDefault()
...
<style lang="scss" scoped>
  .drag_verify {
    ...
    .dv_handler {
      ...
      &:focus-visible {
        z-index: 2;
        outline: 3px solid var(--fluent-interactive, #367a27);
        outline-offset: -3px;
      }
