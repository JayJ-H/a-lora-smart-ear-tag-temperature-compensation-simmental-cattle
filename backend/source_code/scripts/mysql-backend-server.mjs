...
async function ensureV2DatabaseSchema() {
  if (!v2SchemaPromise) {
    v2SchemaPromise = (async () => {
      await applyLocalMigration('database/mysql/migrations/020_v2_full_rebuild_schema.sql')
      await applyLocalMigration('database/mysql/migrations/033_restrict_cattle_breed_scope.sql')
      for (const table of V2_DATABASE_TABLES) {
...
async function appendCowSuggestionRows(output, tableName, keyword, limit) {
  ...
  const earTagExpr = sqlFirstTextExpr(
    meta,
    ['earTagNumber', 'ear_tag_number', 'earTag', 'ear_tag'],
    "''"
  )
...
function buildCowApiPayload(method, args, rawData) {
  if (
    ...
    Object.prototype.hasOwnProperty.call(rawData, 'data')
  ) {
    return rawData
  }
...
async function handleCowScope(scope, method, args, req = null) {
  ...
  if (scope === 'hardware' && method === 'getDeviceDataStream') {
    const rows = await getTableRows('sensors', {
      where: { cowId: first.cowId },
      startTime: first.startTime,
      endTime: first.endTime,
  ...
  if (scope === 'hardware' && method === 'sendDeviceCommand') {
    await ensureProductionAcceptanceTables()
  ...
  if (scope === 'hardware' && method === 'getSystemHealthReport') {
    const status = await buildSystemStatus({ get: () => '' })
    return {
  ...
  if (scope === 'hardware' && method === 'deleteHardwareDevice') {
  ...
  if (scope === 'hardware' && method === 'acknowledgeHardwareAlert') {
