...
const env = {
  ...
  ...readEnvFile(path.join(rootDir, 'ops', 'production', '.env.prod')),
  ...process.env
}
...
async function runProductionActions(headers, baselineRows) {
  ...
  const hardwareCommand = await cowApi(headers, 'hardware', 'sendDeviceCommand', [
    field(device, 'id'),
    {
...
async function main() {
  assert(password, 'ADMIN_PASSWORD is required. Set it in ops/production/.env.prod or environment.')
