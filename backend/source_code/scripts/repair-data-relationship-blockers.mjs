...
function connectionAction(error) {
  ...
  const base = `Set or correct explicit ${auditNames} for a read-capable audit account; these override .env fallbacks and this script never reads ops/production/.env.prod.`
...
function help() {
  return `Usage:
  node scripts/repair-data-relationship-blockers.mjs [--dry-run]
  node scripts/repair-data-relationship-blockers.mjs --apply --allow-non-production-apply

Default mode is dry-run. The script never loads ops/production/.env.prod.
...
On connection failure, dry-run writes a sanitized diagnostic with host/port/user/database/passwordConfigured and env sources.
...
function buildHardwareRow(nowSql, cowTraceScope) {
  ...
  return {
    ...
    configuration_json: json({
      source: 'scripts/repair-data-relationship-blockers.mjs',
