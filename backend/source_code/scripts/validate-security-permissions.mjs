...
function unwrapApi(body) {
  if (body && typeof body === 'object' && 'data' in body && 'code' in body) return body.data
  return body
}
...
async function startTemporaryBackend() {
  ...
  const child = spawn(process.execPath, [path.join(projectRoot, 'scripts', 'mysql-backend-server.mjs')], {
    cwd: projectRoot,
